...
class ReplayBuffer(object):
    # Crea buffer memoria tipo cola (quee)
    def __init__(self, capacity):                 
        self.buffer = deque(maxlen=capacity)
        
    # Guarda una transicion completa en el buffer adaptando estados para facilitar batching.
    def push(self, state, action, reward, next_state, done):
        state      = np.expand_dims(state, 0)       # añade dimension extra al estado (C,H,W) -> (1,C,H,W)
        next_state = np.expand_dims(next_state, 0)  # idem. se hace para luego concatenar varios estados en un batch
        
        self.buffer.append((state, action, reward, next_state, done)) # añadir tupla sarsd a la memoria 
    
    # Selecciona aleatoriamente un batch de experiencias del buffer
    ...
    # Devuelve nº experiencias almacenadas en el buffer
    def __len__(self):
...
# Time difference loss calculation. Actualiza la red principal con nuevos pesos para un batch de transiciones.
def compute_td_loss(c_model, t_model, optimizer, replay_buffer, device, batch_size=32, gamma=0.99): 
    # Obtenemos batch aleatorio de transiciones sarsd del buffer de memoria.
    ...
    # Calculamos Q(s,a) con la CNN principal
    q_values = c_model(state)
    q_value = q_values.gather(1, action.unsqueeze(1)).squeeze(1)

    mean_q_batch = q_values.mean().item()

    # Calculamos Q(s',a') con la CNN target
    with torch.no_grad():                        # Evitamos que PyTorch calcule gradientes en este punto
        next_q_values = t_model(next_state)      # cargamos batch estados siguientes en la red y obtenemos las 6 opciones Q(s',a')
        next_q_value = next_q_values.max(1)[0]   # Seleccionamos valores Q maximos del batch. Tensor 1D de dimension igual al batch.

    # Calculamos valor Q esperado (Bellman)
    expected_q_value = reward + gamma * next_q_value * (1 - done)

    # MAE del batch (estilo Keras) 
    mae_batch = torch.abs(q_value - expected_q_value).mean().item()

    # Calculamos la perdida TD (MSE): L= E[(Q(s,a) -Qtarget)^2]
    loss = torch.nn.functional.mse_loss(q_value, expected_q_value)
    
    # Backpropagation y actualizacion de pesos redes
    optimizer.zero_grad()    # Limpiamos gradientes previos
    loss.backward()          # Calculamos el gradiente de la perdida para todos los pesos de CNN
    optimizer.step()         # Actualizamos los pesos de la red principal (c_model)

    # Devolvemos la perdida, mean-q y mae del batch
    return [loss.item(), mean_q_batch, mae_batch]
